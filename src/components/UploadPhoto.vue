    ...
    <form action="">
        <div class="modal-card" style="width: 640">
            ...
            <section class="modal-card-body">
                <div class="columns">
                    ...
                    <div class="column has-text-left">
                        <b-field label="Name">
                            <b-input size="is-small" v-model="photoObject.name" :disabled="!imagePreview"></b-input>
                        </b-field>
                        <!-- {{photoObject.name}} -->
                        <b-field label="Type">
                            <b-select size="is-small" v-model="photoObject.pType" placeholder="Select a type" :disabled="!imagePreview">
                                <option
                                    v-for="option in phTypes"
                                    :value="option"
                                    :key="option">
                                    {{ option }}
                                </option>
            ...
            <footer class="modal-card-foot has-text-right">
                ...
                <button class="button is-primary" type="button" @click.stop="uploadPhoto">Done</button>
            </footer>
        </div>
    </form>
</template>
...
export default class UploadPhoto extends Vue {
  ...
  private uploadPhoto() {
      ...
      axios.post(`http://localhost:5000/api/image/uploadphoto/${this.photoObject.name}/${this.photoObject.pType}`, fd, {
        headers: {
